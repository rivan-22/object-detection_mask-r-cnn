...
def draw_box(image, class_id, confidence, left, top, right, bottom, class_mask):
    ...
    # Pakai warna acak untuk membedakan tiap instance object.
...
image = cv2.imread(str(IMAGES_DIR / "road.jpg"))
if image is None:
    raise FileNotFoundError(f"Gambar tidak ditemukan: {IMAGES_DIR / 'road.jpg'}")

blob = cv2.dnn.blobFromImage(image, swapRB=True, crop=False)
NET.setInput(blob)

boxes, masks = NET.forward(["detection_out_final", "detection_masks"])
post_process(image, boxes, masks)

OUTPUT_FILE = BASE_DIR / "output_detection.jpg"
cv2.imwrite(str(OUTPUT_FILE), image)
print(f"✓ Deteksi selesai! Hasil disimpan ke: {OUTPUT_FILE}")

cv2.imshow("Object Detection - Mask R-CNN", image)
